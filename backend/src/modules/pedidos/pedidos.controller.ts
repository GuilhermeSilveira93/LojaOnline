...
export class PedidosController {
  constructor(private service: PedidosService, private carrinho: CarrinhoService, private readonly email: EmailService) { }

  @Post()
  async criar(@Request() req, @Body() dto: CriarPedidoDto) {
    const userId = req.user.sub
    const carrinho = await this.carrinho.findUnique(userId);
    if (!carrinho || carrinho?.CarrinhoItem.length === 0) throw new BadRequestException('Carrinho vazio')
    return await this.service.criarAPartirDoCarrinho(userId, dto.clienteId, carrinho.CarrinhoItem);
  }
  ...
  @Patch(':id/status')
  async atualizarStatus(@Request() req, @Param('id') id: string, @Body() dto: StatusPedidoDto) {
    const pedido = await this.service.atualizarStatus(req.user.sub, id, dto.status);
    this.email.NovoStatusPedido(pedido.Cliente, dto.status)
  }
}
