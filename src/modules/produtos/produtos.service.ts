...
export class ProdutosService {
  ...
  async create(dto: CriarProdutoDto) {
    const [produto, produtoError] = await tryCatch(this.db.produto.create({
      data: dto,
    }))
    if (!produto || produtoError) throw new BadRequestException('Erro ao criar produto.');
    return {
      success: true,
      data: produto,
  ...
  async findMany() {
    ...
    if (produtosError || !produtos) throw new NotFoundException('Nenhum produto encontrado.');
    return {
      success: true,
  ...
  async findUnique(id: string) {
    ...
    if (!produto || produtoError) throw new NotFoundException('Nenhum produto encontrado.');
  ...
  async update(id: string, dto: AtualizarProdutoDto) {
    ...
    if (produtoError || !produto) throw new NotFoundException('Produto não encontrado.');
    return {
      success: true,
      data: produto,
